                    ...
                    <div className="mb-2">
                      ...
                      <div className="space-y-1">
                        {event.roster.map((assignment, idx) => {
                          // Get availability for this specific role
                          const roleAvailability = availabilityData[assignment.role] || []
                          const availableCount = roleAvailability.filter(m => m.available).length
                          const totalCount = roleAvailability.length
                          
                          return (
                            <div key={idx} className="flex items-center gap-1.5 text-xs">
                              <span className={`px-1.5 py-0.5 text-xs font-medium rounded ${roleColorMap[assignment.role]}`}>
                                {assignment.role}
                              </span>
                              <span className="text-gray-600 truncate flex items-center gap-1">
                                {getMemberDisplay(assignment.member_id)}
                                {assignment.isGenerated && assignment.member_id && (
                              ...
                              {!assignment.member_id && (
                                <span className={`ml-auto text-xs font-medium ${
                                  availableCount === totalCount ? 'text-green-700' :
                                  availableCount === 0 ? 'text-red-600' :
                                  'text-amber-600'
                                }`}>
